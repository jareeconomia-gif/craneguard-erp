...
const { prepareOfflineSchema, installOfflineAuditHook, installOfflineRoutesHook } = require('./offline-sync-backend');
const { installPortalReportingHook } = require('./portal-reporting-backend');
...
function installFrontendProduction(){const files=['reporting-actions-99.js','clients-production.js','production-scope.js','operations-production.js','offline-production.js','enterprise-completion.js','production-branding.js','sw-production.js'];const enterpriseSource=path.join(__dirname,'enterprise-production.js');if(!files.every(f=>fs.existsSync(path.join(__dirname,f)))||!fs.existsSync(enterpriseSource)){console.warn('CraneGuard: faltan archivos del frontend de producción.');return}const publicDir=path.join(__dirname,'public');if(!fs.existsSync(publicDir))fs.mkdirSync(publicDir,{recursive:true});for(const f of files)fs.copyFileSync(path.join(__dirname,f),path.join(publicDir,f));writeEnterprise(path.join(publicDir,'enterprise-production.js'));const targets=[path.join(__dirname,'index.html'),path.join(publicDir,'index.html')],version='11.3.0',names=['reporting-actions-99.js','clients-production.js','production-scope.js','operations-production.js','enterprise-production.js','offline-production.js','enterprise-completion.js','production-branding.js'],tags=names.map(n=>`<script src="/${n}?v=${version}"></script>`).join('');for(const file of targets){if(!fs.existsSync(file))continue;let html=fs.readFileSync(file,'utf8');for(const n of [...names,'reporting-publish-fix.js','reporting-actions-98.js','production-clean-100.js']){const escaped=n.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');html=html.replace(new RegExp(`<script[^>]+${escaped}[^>]*><\\/script>`,'gi'),'')}html=html.replace(/\/reporting-production\.js\?v=[^"']+/g,`/reporting-production.js?v=${version}`);html=html.includes('</body>')?html.replace('</body>',`${tags}</body>`):html+tags;fs.writeFileSync(file,html,'utf8')}console.log(`CraneGuard: frontend integral ${version} instalado.`)}
function installStartupGate(readyPromise){const previous=express.application.listen;express.application.listen=function(...args){const app=this;readyPromise.then(()=>{console.log('CraneGuard: dependencias productivas listas; abriendo puerto.');previous.apply(app,args)}).catch(e=>{console.error('CraneGuard: no se pudo completar el esquema productivo:',e);process.exit(1)});return app}}
...
(async()=>{try{
  ...
  await prepareOfflineSchema();
  installClientsApiHook(); installOperationsApiHook(); installEnterpriseApiHook(); installPortalReportingHook(); installOfflineRoutesHook(); installOfflineAuditHook();
